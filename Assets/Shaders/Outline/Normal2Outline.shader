	...
	Properties
	{
		...
		SubShader
		{
			...
			Pass
			{
					...
					{
						v2f o;
						float3 pos = normalize(v.vertex);
						half angle = abs(dot(pos, v.normal));
						int a = angle - _Angle; //step(angle, _Angle);
						
						o.pos = UnityObjectToClipPos(v.vertex);
						float3 vnormal = mul((float3x3)UNITY_MATRIX_IT_MV, v.normal);
						float2 offset = TransformViewToProjection(vnormal.xy);

						float2 vv = (a*vnormal + (1 - a)*o.pos.xy);
						o.pos.xy += vv * _OutlineFactor;

						//v.vertex.xyz += (a*v.normal + (1-a)*pos) * _OutlineFactor;
						//o.pos = UnityObjectToClipPos(v.vertex);
