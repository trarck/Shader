...
inline float2 _ClassicPerlinFade(float2 t) { return t * t*t*(t*(t*6.0 - 15.0) + 10.0); }
inline float3 _ClassicPerlinFade(float3 t) { return t * t*t*(t*(t*6.0 - 15.0) + 10.0); }
inline float4 _ClassicPerlinPermute(float4 x) { return fmod(((x*34.0) + 1.0)*x, 289.0); }
inline float4 _ClassicPerlinTaylorInvSqrt(float4 r) { return 1.79284291400159 - 0.85373472095314 * r; }
...
//	by Stefan Gustavson
//
float ClassicPerlinNoise(float4 P) {
	...
	float4 ixy0 = _ClassicPerlinPermute(ixy + iz0);
	float4 ixy1 = _ClassicPerlinPermute(ixy + iz1);
	float4 ixy00 = _ClassicPerlinPermute(ixy0 + iw0);
	float4 ixy01 = _ClassicPerlinPermute(ixy0 + iw1);
	float4 ixy10 = _ClassicPerlinPermute(ixy1 + iw0);
	float4 ixy11 = _ClassicPerlinPermute(ixy1 + iw1);
	...
	float4 norm00 = _ClassicPerlinTaylorInvSqrt(float4(dot(g0000, g0000), dot(g0100, g0100), dot(g1000, g1000), dot(g1100, g1100)));
	g0000 *= norm00.x;
	g0100 *= norm00.y;
	g1000 *= norm00.z;
	g1100 *= norm00.w;

	float4 norm01 = _ClassicPerlinTaylorInvSqrt(float4(dot(g0001, g0001), dot(g0101, g0101), dot(g1001, g1001), dot(g1101, g1101)));
	g0001 *= norm01.x;
	g0101 *= norm01.y;
	g1001 *= norm01.z;
	g1101 *= norm01.w;

	float4 norm10 = _ClassicPerlinTaylorInvSqrt(float4(dot(g0010, g0010), dot(g0110, g0110), dot(g1010, g1010), dot(g1110, g1110)));
	g0010 *= norm10.x;
	g0110 *= norm10.y;
	g1010 *= norm10.z;
	g1110 *= norm10.w;

	float4 norm11 = _ClassicPerlinTaylorInvSqrt(float4(dot(g0011, g0011), dot(g0111, g0111), dot(g1011, g1011), dot(g1111, g1111)));
	g0011 *= norm11.x;
	g0111 *= norm11.y;
	g1011 *= norm11.z;
	g1111 *= norm11.w;
	...
	float4 fade_xyzw = _ClassicPerlinFade(Pf0);
...
float ClassicPerlinNoise(float4 P, float4 rep) {
